...
#define THREADS 8
	...
	static void CacheFontAtlas(const std::string& fontName, float fontSize, AtlasHeader header, const void* pixels)
	{
		...
		{
			stream.close();
			SP_CORE_ERROR("Failed to cache font atlas to {0}", filepath.string());
			return;
		}
		...
		: m_Filepath(filepath), m_MSDFData(new MSDFData())
	{
		...
		if (int remaining = atlasPacker.pack(m_MSDFData->Glyphs.data(), (int)m_MSDFData->Glyphs.size()))
		{
			...
			{
				SP_CORE_ERROR("Error: Could not fit {0} out of {1} glyphs into the atlas.", remaining, (int)m_MSDFData->Glyphs.size());
				SP_CORE_ASSERT(false,  "");
			}
		}
		...
		if (!fixedScale)
			SP_CORE_TRACE("Glyph size: {0} pixels/EM", config.emSize);
		if (!fixedDimensions)
			SP_CORE_TRACE("Atlas dimensions: {0} x {1}", config.width, config.height);
