...
namespace Sparky {
	...
	{
		static constexpr inline uint32_t MaxPrimitives = 20'000;
		static constexpr inline uint32_t MaxVertices = MaxPrimitives * VERTICES_PER_CUBE;
		static constexpr inline uint32_t MaxIndices = MaxPrimitives * INDICES_PER_CUBE;
	...
	void Renderer::Init()
	{
		...
		s_Data.SphereVB = VertexBuffer::Create(RendererInternalData::MaxVertices * sizeof(SphereVertex));
		s_Data.SphereVB->SetLayout({
			{ ShaderDataType::Float3, "a_Position" },
			{ ShaderDataType::Float3, "a_Normal"   },
			{ ShaderDataType::Float4, "a_Color"    },
			{ ShaderDataType::Float2, "a_TexCoord" },
		...
		s_Data.SphereVertexBufferBase = new SphereVertex[RendererInternalData::MaxVertices];

		uint32_t* sphereIndices = new uint32_t[RendererInternalData::MaxIndices];
		...
		// Sphere indices
		{
			...
			for (unsigned int y = 0; y < SPHERE_Y_SEGMENTS; ++y)
			{
				if (!oddRow) // even rows: y == 0, y == 2; and so on
				{
					...
					{
						sphereIndices[i] = (y * (SPHERE_X_SEGMENTS + 1) + x);
						i++;
						sphereIndices[i] = ((y + 1) * (SPHERE_X_SEGMENTS + 1) + x);
						i++;
					}
				}
				else
				{
					for (int x = SPHERE_X_SEGMENTS; x >= 0; --x)
					{
						sphereIndices[i] = ((y + 1) * (SPHERE_X_SEGMENTS + 1) + x);
						i++;
						sphereIndices[i] = (y * (SPHERE_X_SEGMENTS + 1) + x);
		...
		SharedRef<IndexBuffer> sphereIB = IndexBuffer::Create(sphereIndices, RendererInternalData::MaxIndices);
		s_Data.SphereVA->SetIndexBuffer(sphereIB);
		delete[] sphereIndices;

		s_Data.SphereShader = Shader::Create(SPHERE_SHADER_PATH);
		s_Data.SphereShader->Enable();

		s_Data.SphereShader->SetIntArray("u_Textures", samplers, RendererInternalData::MaxTextureSlots);
	...
	void Renderer::BeginScene(const Camera& camera, const Math::mat4& transform)
	{
		...
		s_Data.SphereShader->Enable();
		s_Data.SphereShader->SetMat4("u_ViewProjection", viewProjection);
	...
	void Renderer::BeginScene(const EditorCamera& camera)
	{
		...
		s_Data.SphereShader->Enable();
		s_Data.SphereShader->SetMat4("u_ViewProjection", camera.GetViewProjection());
	...
	void Renderer::Flush()
	{
		...
		if (s_Data.SphereIndexCount)
		{
			...
			// Bind a shader and make a draw call
			s_Data.SphereShader->Enable();
			s_Data.SphereVA->Bind();
			RenderCommand::DrawTriangleStrip(s_Data.SphereVA, s_Data.SphereIndexCount);
			s_Data.RendererStatistics.DrawCalls++;
		}
	}
	...
	void Renderer::AddToSphereVertexBuffer(const Math::mat4& transform, const Math::vec4& color, float textureIndex, float textureScale, int entityID)
	{
		static std::vector<Math::vec3> positions;
		static std::vector<Math::vec3> normals;
		static std::vector<Math::vec2> textureCoords;
		...
		if (positions.empty())
		{
			for (unsigned int x = 0; x <= SPHERE_X_SEGMENTS; ++x)
			{
				for (unsigned int y = 0; y <= SPHERE_Y_SEGMENTS; ++y)
				{
					float xSegment = (float)x / (float)SPHERE_X_SEGMENTS;
					float ySegment = (float)y / (float)SPHERE_Y_SEGMENTS;
					float xPos = Math::Cos(xSegment * 2.0f * Math::PI) * Math::Sin(ySegment * Math::PI);
					float yPos = Math::Cos(ySegment * Math::PI);
					float zPos = Math::Sin(xSegment * 2.0f * Math::PI) * Math::Sin(ySegment * Math::PI);

					positions.push_back(Math::vec3{ xPos, yPos, zPos } * 0.5f);
					normals.push_back(Math::vec3{ xPos, yPos, zPos });
					textureCoords.push_back(Math::vec2{ xSegment, ySegment });
				}
			}
			...
			for (unsigned int y = 0; y < SPHERE_Y_SEGMENTS; ++y)
			{
				if (!oddRow) // even rows: y == 0, y == 2; and so on
				{
					for (unsigned int x = 0; x <= SPHERE_X_SEGMENTS; ++x)
					{
						indices.push_back(y * (SPHERE_X_SEGMENTS + 1) + x);
						indices.push_back((y + 1) * (SPHERE_X_SEGMENTS + 1) + x);
					}
				}
				else
				{
		...
		{
			s_Data.SphereVertexBufferPtr->Position = transform * Math::vec4(positions[i], 1.0f);
			s_Data.SphereVertexBufferPtr->Normal = normals[i];
			s_Data.SphereVertexBufferPtr->Color = color;
			s_Data.SphereVertexBufferPtr->TexCoord = textureCoords[i];
			s_Data.SphereVertexBufferPtr->TexIndex = textureIndex;
			s_Data.SphereVertexBufferPtr->TexScale = textureScale;
...
#if SP_RENDERER_STATISTICS
		s_Data.RendererStatistics.QuadCount += (vertexCount / 4);
#endif // SP_RENDERER_STATISTICS
	}
	...
	void Renderer::DrawSphere(const Math::mat4& transform, const MeshRendererComponent& meshRenderer, int entityID)
	{
		SP_PROFILE_FUNCTION();

		if (s_Data.SphereIndexCount >= RendererInternalData::MaxIndices)
			NextBatch();
		...
		AddToSphereVertexBuffer(transform, meshRenderer.Color, textureIndex, meshRenderer.Scale, entityID);
	}
