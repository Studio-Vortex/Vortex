...
namespace Sparky {
	...
	public:
		...
		static void DrawModel(const TransformComponent& transform, const SharedRef<Model>& model, const SharedRef<Texture2D>& texture, const Math::vec4& color, float scale = 1.0f, bool reflective = false, int entityID = -1);
