...
namespace Sandbox {
	...
	public class Paddle : Entity
	{
		...
		{
			if (Input.IsKeyDown(KeyCode.W) || Input.GetGamepadAxis(GamepadAxis.LeftTrigger) > 0.25f)
				m_Velocity.Y = 1.0f * -Input.GetGamepadAxis(GamepadAxis.LeftTrigger);
			if (Input.IsKeyDown(KeyCode.S) || Input.GetGamepadAxis(GamepadAxis.LeftTrigger) < -0.25f)
				m_Velocity.Y = -1.0f * Input.GetGamepadAxis(GamepadAxis.LeftTrigger);
